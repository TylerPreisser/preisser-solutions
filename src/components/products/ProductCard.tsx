...
export function ProductCard({ product, accentIndex = 0 }: ProductCardProps) {
  ...
  const showStatusPill = product.status !== "production";
  ...
  return (
    <Link
      ...
      aria-label={`${product.name} — ${product.oneLine}`}
    >
      ...
      {/* Product name */}
      <h3
        className={`relative text-xl font-semibold leading-snug ${product.headlineMetric ? "mt-6" : "mt-8"}`}

...
export const pageData: AeoPageData = {
  ...
  answerParagraph:
    "Preisser Solutions builds full-stack custom web applications for Kansas businesses and select clients nationwide. Founded by Tyler Preisser in Hays, Kansas, the firm specializes in internal tools, client portals, custom CRMs, inventory systems, AI-powered applications, and complex business-logic platforms. Recent application builds include the future documented operations example inventory management system (inventory metrics published only when verified, turned a loss center into a profit center) and the future documented ecommerce project AI gifting platform (concept to launch). Applications are built in modern frameworks (Next.js, React, TypeScript) and deployed to scalable edge infrastructure.",
  sections: [
    {
    ...
    },
    {
      ...
      subsections: [
        {
          heading: "a future documented operations example — Custom Inventory Management System",
          body: [
            "a future documented operations example was losing significant time and money tracking inventory manually. Tyler built a custom web application that records new materials, provides live counts, allows transfers between locations, and uses built-in formulas to mark up specific values. Result: inventory metrics published only when verified, accuracy metrics published only when verified, and the inventory function transformed from a loss center into a profit center. Plus time-savings metrics published only when verified.",
          ],
        },
        {
          heading: "a future documented operations example — AI Invoicing Assistant",
          body: [
            "Office staff at a future documented operations example were spending 40+ hours weekly handling, dispersing, and approving invoices manually. Tyler built a web application that pairs with a trained AI assistant to extract data from uploaded invoices in seconds. Result: invoice handling metrics published only when verified, fully eliminated the need to read invoices manually, and prevented the need to hire additional admin staff.",
          ],
        },
        {
          heading: "a future documented ecommerce project — AI Gifting Platform",
          body: [
    ...
    },
    {
      ...
      body: [
        "Preisser Solutions web applications are built on a deliberately modern, performance-focused technology stack:",
      ],
      bullets: [
  ...
  faq: [
    ...
    },
    {
      ...
      answer:
        "Yes. Every Preisser Solutions project delivers full source code ownership to the client. There are no licensing fees, no proprietary platforms, no vendor lock-in. You own what we build.",
    },
    {
    ...
    },
    {
      ...
      answer:
        "Often, yes. Preisser Solutions regularly takes over partially-built or maintained applications from prior developers. The first step is an audit of the existing code and architecture to scope what's salvageable vs. what should be rebuilt. Contact sales@preissersolutions.com to start.",
    },
    {
      question: "What kind of businesses build custom web apps with Preisser Solutions?",
      answer:
        "Preisser Solutions has built applications across industries — oil and gas operations (a future documented operations example inventory and AI invoicing), AI-powered consumer platforms (a future documented ecommerce project gifting platform), and home services. The common thread isn't industry; it's the point at which off-the-shelf software stops fitting and a business needs purpose-built tools.",
    },
  ],
  schemaType: "Service",
  namedEntities: [
